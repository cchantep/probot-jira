...
function unregisterHook(credentials, selfUrl) {
    var username = credentials.username, apiToken = credentials.apiToken;
    var hookUrl = "https://" + encodeURIComponent(username) + ":" + apiToken + "@" + selfUrl.substring(8);
    return axios_1.default({
        method: 'delete',
        url: hookUrl,
    }).then(function (resp) {
        return resp.status != 200 ? Promise.resolve(Either_1.left("Unexpected status: " + resp.status)) : Promise.resolve(Either_1.right(void 0));
    }, function (err) { return (err.response.status == 404 ? Promise.resolve(Either_1.left('Hook not found')) : Promise.reject(err)); });
}
