...
function checkIsClosed(context, config, repoInfo, credentials, author, pr) {
    return withJiraIssue(context, repoInfo, pr, config, function (data) {
        ...
        var jstatus1 = issue.fields.status.name;
        var jstatus2 = issue.fields.status.untranslatedName;
        if (config.postMergeStatus.find(function (s) { return (s == jstatus1 || s == jstatus2); })) {
            return Promise.resolve(context.log("JIRA issue " + issue.key + " for pull request #" + pr.number + " is now ['" + jstatus1 + "', '" + jstatus2 + "']"));
        }
        else {
            var details = config.postMergeStatus.join(', ');
            var msg = "JIRA issue [" + issue.key + "](" + url + ") doesn't seem to have a valid status: ['" + jstatus1 + "', '" + jstatus2 + "'] !~ [" + details + "]";
            context.log(msg + " (pull request #" + pr.number + ")");
            return context.github.issues
                .createComment(__assign(__assign({}, repoInfo), { issue_number: pr.number, body: "@" + author + " " + msg + ". Please check it." }))
    ...
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    repo = context.repo({});
                    return [4 /*yield*/, c.getConfig(context, repo, pr.head.ref)];
                case 1:
                    config = _a.sent();
...
function checkMilestone(context, repo, config, pr, issue, issueUrl) {
    return __awaiter(this, void 0, void 0, function () {
        var re, minfo, msg, milestone, found, details, description;
        return __generator(this, function (_a) {
            if (!pr.milestone) {
            ...
            re = config.milestoneRegex || '^(.+)$';
            minfo = pr.milestone.title.match(re);
            context.log.debug("milestoneRegex = " + re);
            if (!minfo || minfo.length < 2) {
                msg = "Milestone " + pr.milestone.title + " doesn't match " + re;
                context.log(msg);
                return [2 /*return*/, toggleState(context, repo, StatusContext, pr.head.sha, 'error', msg, Option_1.none)];
            }
            milestone = minfo[1];
            ...
            if (found >= 0) {
                ...
                context.log("No JIRA fixVersion for issue '" + issue.key + "' is matching the milestone '" + pr.milestone.title + "' of pull request #" + pr.number + ": " + details);
                description = ("Milestone '" + pr.milestone.title + "' doesn't match '" + issue.key + "' fixVersion " + details + ": " + config.fixVersionRegex).substring(0, 140);
...
function runUrl(pr) {
    return pipeable_1.pipe(Option_1.fromNullable(process.env['GITHUB_RUN_ID']), Option_1.map(function (id) {
        var i = pr.html_url.indexOf('/pull/');
        var repoUrl = pr.html_url.substring(0, i);
                    ...
                    switch (_a.label) {
                        ...
                        case 1:
                            ...
                            return [4 /*yield*/, j.getIssue(credentials, k).then(function (res) {
                                ...
                                }, function (err) { return either.left(err.message); })];
                        case 2:
                            jiraIssue = _a.sent();
...
function toggleState(bot, repo, statusContext, sha, expectedState, msg, url) {
    return getCommitStatus(bot, repo, sha, statusContext).then(function (st) {
        var mustSet = expectedState == 'success'
            ? isSuccessful(st)
            : !Option_1.exists(function (s) {
                return s.state == expectedState &&
                    s.description == msg &&
                    pipeable_1.pipe(url, Option_1.fold(function () { return s.target_url == null; }, function (u) { return u == s.target_url; }));
            })(st);
        if (!mustSet) {
...
function getCommitStatus(bot, repo, ref, ctx) {
    return bot.github.repos.listStatusesForRef(__assign(__assign({}, repo), { ref: ref })).then(function (resp) {
...
module.exports = function (app) {
    app.on(['pull_request.opened', 'pull_request.edited', 'pull_request.synchronize', 'pull_request.reopened'], function (context) { return __awaiter(void 0, void 0, void 0, function () {
        var event, pr;
        return __generator(this, function (_a) {
            switch (_a.label) {
            ...
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, context.github.pulls
                            .get(__assign(__assign({}, repoInfo), { pull_number: prNumber }))
                            .then(function (resp) { return util_1.fromEither(pullrequest_1.PullRequestInfo.decode(resp.data)); })];
                case 1:
                    pr = _a.sent();
                    return [4 /*yield*/, c.getConfig(context, repoInfo, pr.head.ref)];
                case 2:
                    config = _a.sent();
                    return [4 /*yield*/, jira.credentials(repoInfo.owner, repoInfo.repo)];
                case 3:
    ...
    app.on('issues.demilestoned', function (context) {
        return withIssuePR(context, function (pr) { return __awaiter(void 0, void 0, void 0, function () {
            var repo, config;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
    ...
    app.on('repository_dispatch', function (context) { return __awaiter(void 0, void 0, void 0, function () {
        function find(items) {
            return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 2:
                    ...
                    return [4 /*yield*/, context.github.pulls.list(__assign(__assign({}, repoInfo), { state: 'open' }))];
                case 3:
                    resp = _b.sent();
    ...
    app.on('schedule', function (context) { return __awaiter(void 0, void 0, void 0, function () {
                ...
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        ...
                        case 2:
                            ...
                            return [2 /*return*/, checkIsClosed(context, config, repoInfo, credentials, pr.user.login, pr).then(function (_r) {
                                    return check(items.slice(1));
                                })];
                    }
                });
            });
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    ...
                    return [4 /*yield*/, context.github.pulls.list(__assign(__assign({}, repoInfo), { state: 'closed', sort: 'updated', direction: 'desc', per_page: 50 }))];
                case 1:
                    resp = _a.sent();
    ...
    app.on("*", function (context) { return __awaiter(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            ...
            p = context.payload;
            branch = p['pull_request'] && p.pull_request && p.pull_request['head']
                ? p.pull_request.head.ref
                : p['repository']
                    ? p.repository.default_branch
                    : null;
            cfg = branch ? c.getConfig(context, repoInfo, branch) : c.getConfig(context, repoInfo, 'master');
            cfg.then(function (config) {
                var baseUrl = config.githubDispatchBaseUrl || 'https://pr-jira-gh.herokuapp.com';
